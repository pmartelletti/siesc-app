...
================================================== */
	...
	oTable = $('.datatable-selectable table').dataTable({
		...
		"oTableTools": {
			...
			[
				{
					"sExtends": "collection",
					"sButtonText": "Herramientas <span class='caret'></span>",
	...
	oTable = $('.datatable-tools table').dataTable({
		...
		"oTableTools": {
			...
			"aButtons": [
				...
				{
					"sExtends":    "collection",
					"sButtonText": "Exportar <span class='caret'></span>",
					"sButtonClass": "btn btn-primary",
	...
	$('.dataTables_filter input[type=text]').attr('placeholder','Escriba para filtrar...');

    $('.datatable-extra-filter').change(function(){
        var index = $(this).attr('data-index');
        $('.datatable-tools table').dataTable({ "bRetrieve": true }).fnFilter(this.value, index);
    })

    // we trigger first change, just in case
    $('.datatable-extra-filter').trigger('change');
...
================================================== */
	...
	$("select.form-control, .select-full").select2({
		//minimumResultsForSearch: "-1",
		width: "100%"
	});
...
//
    ...
    $("ul.navigation li.active").closest("ul").prev().trigger('click');
